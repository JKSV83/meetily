...
use super::default_output_device;
use super::{
    default_input_device, parse_audio_device, DeviceEvent, DeviceMonitorType, RecordingManager,
};
...
#[cfg(target_os = "linux")]
fn resolve_linux_system_device_from_preference(
    name: &str,
) -> Result<Arc<super::AudioDevice>, String> {
    super::devices::platform::linux::resolve_linux_system_audio_device(name)
        .map(Arc::new)
        .map_err(|e| format!("{}", e))
}

#[cfg(target_os = "linux")]
fn default_linux_system_device() -> Option<Arc<super::AudioDevice>> {
    match super::devices::platform::linux::default_linux_system_audio_device() {
        Ok(device) => {
            info!(
                "✅ Using default Linux system audio monitor: '{}'",
                device.name
            );
            Some(Arc::new(device))
        }
        Err(e) => {
            warn!("⚠️ No default Linux system audio monitor available: {}", e);
            None
        }
    }
}
...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    let system_device = match preferred_system_name {
        Some(pref_name) => {
            ...
            #[cfg(target_os = "linux")]
            {
                match resolve_linux_system_device_from_preference(&pref_name) {
                    Ok(device) => {
                        info!("✅ Using Linux system audio monitor: '{}'", device.name);
                        Some(device)
                    }
                    Err(e) => {
                        warn!(
                            "⚠️ Preferred Linux system audio '{}' not available: {}",
                            pref_name, e
                        );
                        warn!("   Falling back to default Linux monitor source...");
                        default_linux_system_device()
                    }
                }
            }
        ...
        None => {
            ...
            #[cfg(target_os = "linux")]
            {
                let device = default_linux_system_device();
                if device.is_none() {
                    warn!("   Recording will continue with microphone only");
                }
                device
...
pub async fn start_recording_with_devices_and_meeting<R: Runtime>(
    ...
    let system_device = if let Some(ref name) = system_device_name {
        ...
        {
            Some(
                resolve_linux_system_device_from_preference(name)
                    .map_err(|e| format!("Invalid Linux system device '{}': {}", name, e))?,
            )
        }
