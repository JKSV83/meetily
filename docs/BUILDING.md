...
1.  **Use the frontend entrypoint:** Run the shell script from repo root as `./frontend/dev-gpu.sh` or `./frontend/build-gpu.sh`
2.  **Auto-detect GPU:** Run `scripts/auto-detect-gpu.js` (or use `TAURI_GPU_FEATURE` if set)
3.  **Build Sidecar:** Run `frontend/scripts/prepare-llama-helper-sidecar.sh` with the detected feature (debug or release)
4.  **Copy Binary:** Stage the built sidecar in `frontend/src-tauri/binaries` with the host target triple
